...
From the properties of a CTMP, it is clear to see that $\tau_n \sim Exp(\lambda)$, meaning that a poisson process can be simulated by a rather simple algorithm.
...
\Output{$\mathbf{t}$, a Poisson process of rate $\lambda$ terminating before time $T$}
...
Since this process is no longer Markovian, it makes little sense to draw its graph, however it remains a powerful tool.
