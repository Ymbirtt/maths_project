...
An MMPP seems ideal then, capturing the simplicity of a step function, and letting us model the idea of randomly distributed bursts throughout a day. Indeed, several authors have postulated that such a model would be ideal for simulating human behaviour %lots of citations go here
...
\Output{$\mathbf{x}$, the most likely sequence states in which the underlying CTMC resides for each emission in $\mathbf{y}$}
...
\begin{figure}[h!]
\centering
\includegraphics[width = 0.4\textwidth]{./images/fit_mmpp2.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width = 0.4\textwidth]{./images/fit_mmpp3.png}
...
\begin{figure}[h!]
\centering
\includegraphics[width = 0.4\textwidth]{./images/fit_dthmm1.png}
...
And now, observing Figure \ref{densities_mmpp1} we see an issue. The fits are close, but the heads of the distributions are light and their tails heavy. Indeed, on inspection, the observed emissions within each state seem closer to a low-variance lognormal distribution - if $X$ follows a lognormal distribution, then $\ln(X)$ follows a normal distribution \cite{mwlognormal}. Plotting a normal density curve alongside the density of the natural logarithm of our observed inter-arrival times, as in Figure \ref{density_mmpp2_state3}, we see something that almost looks like a correlation. Let's try doing this properly.
...
\begin{figure}
\centering
\includegraphics[width = 0.4\textwidth]{./images/density_mmpp2_state3.png}
...
\begin{figure}
\centering
\includegraphics[width = 0.4\textwidth]{./images/fit_dthmm2.png}
