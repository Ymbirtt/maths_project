...
\right)\\
O &= \mathbb{R}^{+}\\
x \in O => f_s(x) &= \frac{\exp({-\frac{(x-\mu_s)^2}{2\sigma_s^2}})}{\sigma_s\sqrt{2\pi}}
\end{align*}

Given a sequence of emissions from this DTHMM, $\mathbf{x}$, the inter-arrival times are then $\mathbf{t}$ with $t_i = e^{x_i}$. Perhaps a little strangely, shading the states onto our graph as in figure %fig
gives a less obvious seasonality to the tweeter's behavior. We still have observable and highlighted bursts as well as some regularity to the user's sleeping patterns, but they're less obvious here than with the MMPP. More work would certainly throw up more results, perhaps this requires a continuous time model for such things to be seen, or perhaps the number of states is too small, 
