...
And now, observing figures \ref{density_mmpp1_state1} to \ref{density_mmpp1_state5} we see an issue. The fits are close, but the heads of the distributions are light and their tails heavy. Indeed, on inspection, the observed emissions within each state seem closer to a low-variance lognormal distribution - if $X$ follows a lognormal distribution, then $\ln(X)$ follows a normal distribution %cite mathworld
